...
def _context_stage(payload: dict[str, Any], config: dict[str, Any]) -> str:
    ...
    return (
        ...
        + f'</select><button id="run-model" class="primary-button" type="button"{"" if _analyzable_hosts(payload) else " disabled"}>Analyze target</button></div>'
...
def _analyst_choices(payload: dict[str, Any]) -> str:
    """Asset options for the analyst panel.

    A host without stored findings cannot form a case, so its option is disabled
    and labelled as such instead of producing a runtime refusal after the click.
    """
