...
def test_fixed_free_model_and_bounded_structured_request():
    ...
    with patch("vulnassess.openrouter.urllib.request.urlopen", return_value=Response({
        "choices": [{"message": {"content": json.dumps(expected)}}]
    })) as send:
        client = OpenRouterClient(api_key="synthetic-test-key")
        assert client.available()
        assert client.generate_structured("synthetic prompt", {"type": "object"}, num_ctx=16384) == expected
...
def test_rate_limit_fails_once_without_retry_or_paid_fallback():
    error = HTTPError("https://openrouter.ai/api/v1/chat/completions", 429, "Too Many Requests", {}, None)
    with patch("vulnassess.openrouter.urllib.request.urlopen", side_effect=error) as send:
        with pytest.raises(LLMUnavailable, match="rate limit"):
            OpenRouterClient(api_key="synthetic-test-key").generate_structured("x", {"type": "object"})
...
def test_free_model_404_explains_paid_only_response_without_leaking_envelope():
    error = HTTPError(
        "https://openrouter.ai/api/v1/chat/completions", 404, "Not Found", {},
...
def test_missing_key_fails_before_network():
    with patch("vulnassess.openrouter.urllib.request.urlopen", side_effect=AssertionError("network")):
